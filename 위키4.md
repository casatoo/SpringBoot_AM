...
- 회원가입 시 아이디 중복 체크
    ...
    - 회원가입에 적용을 했었다
    - 그대로 가져오면 될거같다. 
    - 보낸 아이디가 중복이면 빨간글씨 아니면 초록글씨
    - code 와 msg 를 보내자
    - code에 따라 메세지를 바꾸는것으로
    - 입력되는 길이는 2자 이상으로
...
- 페이지 서칭 중에 회원가입, 로그인 경우에도 페이지 유지되도록함.

- 댓글 작성시 새로고침되면서 페이지가 생성되는문제
    - 비동기로 가져올때 새로고침없이 쌓으려면
    - list 자체를 불러와서 반복문으로 쌓아보면 될듯..

- 회원가입시 id 를 2자이상 입력하지 않았을때 처리
...
- 기능추가 필요한것.
    - 비동기 처리로 변환해야할것.
        - 게시판 페이지 이동
        - 검색
            - 검색을 비동기처리했을때 uri 가 그대로인지..?
        - 댓글 시스템
            - 추가, 삭제, 수정

- 댓글 기능 jspf 로 빼서 따로 관리 하는것으로..
    - 비동기로 하려면
    - comment list 를 받아오는것.
        - 페이지 불러올때 무조건 한번 실행하고
        - 작성 버튼 눌렀을때 작동해서 불러오기
    - 방식 변경
        - 
...
- 컨트롤러 생성
    ...
    async function getData(){
        const url ='http://apis.data.go.kr/B552061/schoolzoneChild/getRestSchoolzoneChild?ServiceKey='+API_KEY +'&searchYearCd=2015&siDo=11&guGun=320&type=xml&numOfRows=10&pageNo=1;
    ...
    ```
    - 요청 주소에 요청변수를 파라미터로 입력해서 url에 저장
    ...
    ```
    - 지도 생성 가져온 값으로 마커 생성
    - 가져오는 데이터의 형식을 잘 파악해야 함. json , 파일 구조


- 관리자 페이지
    - 요구사항
        - 회원목록 생성
            - 게시글 리스트와 같은 형식이면 될거같음,
            - 번호, 
...
## 2022.11.16
- 팝업창 만들기
    - 프론트 엔드에서 했었던 내용 찾아서 참고해도 될거같음.
    - 팝업 창을 하나 생성해서 가려놨다가 버튼 클릭하면 보여지게 함 .show()
    - 배경을 비활성화 시키고 어두운 색으로 나오게 함
    - 팝업 주변 클릭하거나 close 누르면 팝업 꺼지게 함.

- 관리자 페이지 생성
    - 관리자 계정이 아닐경우 접근할수 없도록 인터셉터로 조치
    - 관리자 계정일 경우 nav에 메뉴 표시
    - memberList 불러와서 리스트 출력


## 2022.11.17
- 비밀번호 암호화
    - 데이터베이스에서 암호화
    - mysql 함수 SHA2(암호화하는변수,256);
    - 로그인시 값 확인을 위해 입력값 변환
    - MessageDigest.getInstance("SHA-256");

- 웹에서도 바꿔주는 사이트가 있고 QR 코드도  가능
